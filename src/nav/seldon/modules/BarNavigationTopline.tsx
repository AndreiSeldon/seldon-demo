...
export interface BarNavigationToplineProps extends HTMLAttributes<HTMLElement> {
  className?: string
  button?: ButtonProps
  icon?: IconProps
  label?: LabelProps
 ...
 * ```
 */
export function BarNavigationTopline({
  className = "",
  button = sdn.button,
  icon,
  label,
  ...
  label5,
  ...props
}: BarNavigationToplineProps) {
  return (
    <Frame
      className={"sdn-barNavigation sdn-barNavigation-mui9HS " + className}
      {...props}
    >
      <Button
        {...{ ...sdn.button, ...button }}
...
const sdn: BarNavigationToplineProps = {
  button: {
    className: "sdn-button-TKUeHv",
  },
  icon: {
    icon: "material-home",
